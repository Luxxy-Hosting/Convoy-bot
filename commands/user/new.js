...
module.exports = async (client, message, args) => {
        ...
        if (userDB) return message.channel.send('You already have an account!');
        ...
        let category = message.guild.channels.cache.find(c => c.id === process.env.createAccountCategoryID);
        let channel = await message.guild.channels.create({
            name: `${message.author.username}-account`,
            parent: category.id,
        ...
        let msg = await channel.send({
            ...
            embeds:[
                ...
                .setTitle(`👋 Welcome to ${process.env.GUILD_NAME}`)
                .setColor(0x677bf9)
                .setDescription(`📰 In order to continue, please read our terms of service and privacy policy that are located in the channel TODO.\n\nYou are allowed to continue creating your account and use our servicies only if you accept our terms of service and privacy policy\n\nDo you accept our legal?`)
        ...
        if(reason === 'Success'){
            ...
            collector.on('end', async (collected, reason) => {
                ...
                if(reason === 'FinishedCreation'){
                    ...
                    axios({
                        url: process.env.CONVOY_URL + "/api/application/users",
                        method: 'POST',
                        followRedirect: true,
                        maxRedirects: 5,
                        headers: {
                        ...
                        message.member.roles.add(message.guild.roles.cache.get(process.env.memberRoleID))
                        console.log(user.data.data)
                        UserData({
                            ID: message.author.id,
                            consoleID: user.data.data.id,
